...
gsa = egsea(voom.results=v, contrasts=contrasts,  gs.annots=gs.annots, 
 			symbolsMap=v$genes, baseGSEAs=egsea.base()[-c(2, 12)], display.top = 3,
...
\begin{itemize}
\item The $p$-value score is the average $p$-value assigned to $\gamma_i$ and is
calculated using six different methods. 
...
gsa = egsea(voom.results=v, contrasts=contrasts, gs.annots=gs.annots, 
        symbolsMap=symbolsMap, baseGSEAs=egsea.base()[-c(2,5,6,9,12)], 
...
gsa = egsea.cnt(counts=cnt, group=group, design=design, contrasts=contrasts,  
        gs.annots=gs.annots, symbolsMap=genes, baseGSEAs=egsea.base()[-c(2,12)], display.top = 5,
